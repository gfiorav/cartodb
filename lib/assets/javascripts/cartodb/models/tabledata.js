...
cdb.admin.CartoDBTableData = cdb.ui.common.TableData.extend({
  ...
  _aliasSchemaFromQueryFields: function(fields){
    var asc = {};
    for(var l in fields){
      if(fields[l].alias){
        asc[l] = fields[l].alias
      }
    }
    return asc;
  },
  ...
  */
  ...
  histogram: function(nbuckets, column, callback, error) {
    ...
    'with bounds as ( ' +
     'SELECT  ' +
      ...
      'max(<%= column %>) as upper,  ' +
      '(max(<%= column %>) - min(<%= column %>)) as span,  ' +
      'CASE WHEN ABS((max(<%= column %>) - min(<%= column %>))/<%= nbuckets %>) <= 0 THEN 1 ELSE GREATEST(0.0000000001, pow(10,ceil(log((max(<%= column %>) - min(<%= column %>))/<%= nbuckets %>)))) END as bucket_size ' +
      'FROM  (<%= sql %>) _table_sql  ' +
    ')  ' +
  ...
  getSQL: function() {
    ...
    return "select * from " + this.table.get('id');
  },
  ...
  _fetch: function(opts) {
    ...
    //TODO This is a hacky way to make sure the table data returns before the get schema call.
    //we need the table schema_alias to before normal schema to inject the alias into the columns
    var self = this;
    // setTimeout(function(){
      this._sqlQuery(_.template('select * from (<%= sql %>) __wrapped limit 0')({ sql: sql }), function(data) {
        ...
        var process = function(self) {
          for (var k in tmpSchemaAlias){
            if (tmpSchemaAlias.hasOwnProperty(k)) {
              if (data.fields[k]) {
                data.fields[k].alias = tmpSchemaAlias[k];
              }
            }
          }
          self.query_schema = self._schemaFromQueryFields(data.fields);
          self.alias_query_schema = self._aliasSchemaFromQueryFields(data.fields);
        ...
        var tmpSchemaAlias = self.table.get('schema_alias');
        if(tmpSchemaAlias !== undefined){
          process(self);
        }else{
          self.table.fetch(null, process.bind(this) );
        }
      }, function (err) {
  ...
  schemaFromData: function(originalTableSchema) {
    ...
    for(var k in this.alias_query_schema){
      for(var a = 0; a < schema.length; a++){
        if(k === schema[a][0]){
          schema[a][2] = this.alias_query_schema[k];
        }
      }
    }
    return schema;
