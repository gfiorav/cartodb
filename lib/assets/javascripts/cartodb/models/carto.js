...
function bubble_generator(table, props, changed, callback) {
  ...
  // generate carto
  var table_name = table.getUnqualifiedName();
  var tmpl = _.template('SELECT min(<%= column %>), max(<%= column %>) FROM <%= table %>');
  table.data()._sqlQuery(tmpl({ table: table_name, column: prop }), function(data) {
    var minMaxRow = data.rows;
    simple_polygon_generator(table, carto_props, changed, function(css) {
      table.wizard_data_range = { min: minMaxRow[0].min, max: minMaxRow[0].max};
...
cdb.admin.CartoStyles = Backbone.Model.extend({
    ...
    _generateCarto: function(){
      ...
      this.properties.bind('change', function() {
        //TODO Again This is a hack way of replacing the alias column name with the correct original name
        //Cannot find where the properties.property field is being updated on the rename alias (chane:schema event acctually)
        ...
        if(changed.property_cat){
          for(var k in tableAliasSchema){
            if(tableAliasSchema[k] === changed.property_cat){
              changed.property_cat = k;
            }
          }
        }
      }, this);
      //TODO This is a hack way of replacing the alias column name with the correct original name
      //Cannot find where the properties.property field is being updated on the rename alias (chane:schema event acctually)
      var thisProperties = this.get('properties');
      var tableAliasSchema = this.table.get('schema_alias');
      // for(var k in tableAliasSchema){
      //   if(tableAliasSchema[k] === thisProperties.property){
      //     thisProperties.property = k;
      //   }
      // }
      for(var key in thisProperties){
        for(var key2 in tableAliasSchema){
          if(thisProperties[key] === tableAliasSchema[key2]){
            thisProperties[key] = key2
          }
        }
      }
      this.properties.set(thisProperties);
      ...
      gen(this.table, this.get('properties'), changed, function(style, metadata, sql) {
        ...
        if (metadata) {
          ...
          // update metadata and properties.categories
          self.attributes.properties.categories = metadata;
        }
