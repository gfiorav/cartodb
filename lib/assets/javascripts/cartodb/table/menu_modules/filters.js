...
cdb.admin.mod.Filters = cdb.admin.Module.extend({
    ...
    loadScroll: function() {
      ...
      setTimeout(function() {
        ...
        $scrollPane.css("max-height", self.$el.outerHeight(true)); // was -60 because originially there was a title above "Filters"
    ...
    _addFilter: function(column) {
      ...
      // re-render histogram, because it is only now available to DOM
      if (columnType == 'number' || columnType == 'date') {
        v._renderHist();
      }
