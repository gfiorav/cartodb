...
cdb.admin.FormSchema = cdb.core.Model.extend({
  ...
  _fillColumns: function(opts) {
    ...
    if (self.table.isExternal()) {
      formatName = function(name) { return [Sugar.String.titleize(name[2]), name]; };
    } else {
...
//            extra = extra.concat(_.without(columns, 'cartodb_id').map(formatName));
            extra = extra.concat(_.reject(columns, function(r) { return ['cartodb_id'].indexOf(r[0]) !== -1}).map(formatName));
            if (f.default_column === type) {
//              var customColumns = _.without(columns, 'cartodb_id', 'created_at', 'updated_at');
              var customColumns = _.reject(columns, function(r) { return ['cartodb_id', 'created_at', 'updated_at'].indexOf(r[0]) !== -1});
              if (customColumns.length) {
                f.value = customColumns[0][0];
              }
            }
          }
          if (!f.value) {
...
cdb.admin.WizardProperties = cdb.core.Model.extend({
  ...
  initialize: function() {
    ...
    this.table.bind('change:geometry_types', function() {
      if (!this.isDisabled() && this.table.previous('schema') !== undefined) {
