...
module.exports = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this.bSampleRedirect = true; //do only once per session
  ...
  _initBindings: function() {
    ...
    this.collection.bind('show-loader', this._onShowLoader, this);
  ...
  _initViews: function() {
    ...
    var datasetSelectNoticeView = new ContentResult({
      className:  'ContentResult dataset-select-notice',
      user:       this.user,
      router:     this.router,
      collection: this.collection,
      template:   'dashboard/views/content_dataset_select_notice'
    });

    this.controlledViews['dataset_select_notice'] = datasetSelectNoticeView;
    this.$('.NoDatasets').append(datasetSelectNoticeView.render().el);
    this.addView(datasetSelectNoticeView);
  ...
  _onDataFetched: function() {
    ...
    if (this.router.model.get('content_type') === "datasets" && this.collection.size() > 0) {
      activeViews.push('dataset_select_notice');
    }

    if (this.collection.size() === 0) {
      if (!tag && !q && shared === "no" && !locked && !liked && !samples) {

        if (this.router.model.get('content_type') === "maps") {
          // If there is no maps, let's show create first kmap message
          if(this.bSampleRedirect)
          {
            this.router.navigate(this.router.currentDashboardUrl()+"/samples", {trigger: true});
            this.bSampleRedirect = false;
          }
          else
          {
            activeViews = ['filters', this.collection.total_entries == 0 ? 'list' : 'no_results'];
