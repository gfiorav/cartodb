...
(function() {
  ...
  cdb.admin.CartoDBTableMetadata = cdb.ui.common.TableProperties.extend({
    ...
    aliasNames: function(sc) {
      sc = sc || 'schema_alias'
      return this.get(sc);
    },

    getColumnAlias: function(columnName) {
      var returnValue = null;
      for(var key in this.attributes.schema_alias){
        if(key === columnName){
          returnValue = this.attributes.schema_alias[key];
        }
      }
      return returnValue;
    },
    ...
    columnNamesByTypeAsObj: function(type, sc) {
      sc = sc || 'schema';
      var t = _(this.get(sc)).filter(function(c) {
        return c[1] == type;
      });
      return _.map(t, function(r) {
        return [r[0], r[0], r[2]];
      });
    },
    ...
    renameAlias: function(columnName, aliasOldName, newAliasName, opts) {
      if(aliasOldName === newAliasName) return;
      var aliasList = this.get('schema_alias');
      //When new table is created the schema_alias comes back as a string and not an object.  
      //Convert into object if typeof is not object
      aliasList = (typeof aliasList === 'object') ? aliasList : {};
      if(newAliasName) {
        aliasList[columnName] = newAliasName;
      }else{
        delete aliasList[columnName]
      }
      this.set('schema_alias', aliasList);
      this.set('alias_columns', JSON.stringify(aliasList));
      var self = this;
      this.notice('renaming column alias', 'load', 0);
      this.save(null,  {
          success: function(mdl, data) {
            self.notice('Column alias has been renamed', 'info');
            // self.trigger('columnRename', newAliasName, aliasOldName);
            self.trigger('columnRenameAlias', newAliasName, aliasOldName, columnName);
            self.data().fetch();
            opts && opts.success && opts.success(mdl, data);
          },
          error: function(e, resp) {
     ...
     */
    //TODO Link Column Name to info window
    linkToInfowindow: function(infowindow) {
      ...
      this.bind('columnRenameAlias', function(newAliasName, oldAliasName, columnName) {
        if(infowindow.containsAlias(oldAliasName)) {
          infowindow.removeField(columnName);
          infowindow.addField(columnName, null, newAliasName);
        }
      }, infowindow);
    ...
    duplicate: function(newName, callbacks) {
      ...
      if (this.isInSQLView()) {
        ...
        data.sql = ( !query || query == "" ) ? "SELECT * FROM " + this.get('name') : query;
      } else {
