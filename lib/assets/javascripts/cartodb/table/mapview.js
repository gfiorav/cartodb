...
if (user_data && user_data.feature_flags && user_data.feature_flags.indexOf('bbg_new_search') != -1) {
  SearchInput = Backbone.View.extend({
    ...
    _renderListItem: function(service, index, itemMatch, selected) {
      ...
      return ('<li class="search-result-row' + (selected ? ' selected ' : '') + '" data-index="' + index + '" data-key="' + service + '">' +
                '<div class="name">' + itemMatch.match + '</div>' +
                '<div class="tags">' + itemMatch.dataset + '</div>' +
              '</li>');
    },
  ...
  SearchInput = Backbone.View.extend({
    ...
    _doDatasetSearch: function() {
      ...
      .done(function(response) {
          var items = [];
          var results = response.visualizations;
          var i, numResults = results.length, result;
          for (i = 0; i < numResults && i < self._numMaxResults; ++i) {
              result = results[i];
              items.push({ id: result.id, type: result.type, name: result.name, description: result.description, tags: result.tags });
          }
    ...
    _renderListHeader: function() {
      return '<ul class="search-result-list">';
    },

    _renderListItem: function(index, item, selected) {
      ...
      if (startPos == -1) {
          ...
          name = item.name.substr(0, startPos) + '<span class="found-sub">' + item.name.substr(startPos, queryLen) + '</span>' + item.name.substr(startPos + queryLen);
      }
      
      if (item.tags != null) {
          var tagsArray = item.tags, i, numTags = tagsArray.length, strTags = "", aTag;
          for (i = 0; i < numTags; ++i) {
              aTag = tagsArray[i];
              if (aTag[0] == '"') {
      ...
      if (startPos != -1) {
          tags = tags.substr(0, startPos) + '<span class="found-sub">' + tags.substr(startPos, queryLen) + '</span>' + tags.substr(startPos + queryLen);
      }
      var typeClass = item.type === 'remote' ? ' remote' : '';
      var itemHtml = '<li class="search-result-row ' + (selected ? ' selected ' : ' ') + typeClass + '" data-index="' + index + '"><div class="name">' + name + '</div><div class="tags">';
      if (item.tags != null) {
          itemHtml += tags;
      }
      itemHtml += '</div></li>';
    ...
    _renderNoResult: function() {
      return '<li class="search-result-row no-result"><div class="name">No results found...</div><div class="tags"></div></li>';
    },
    ...
    _onKeyDown: function(e) {
      switch (e.keyCode) {
        case 13: // enter
          e.stopPropagation();
          if (this._items.length > 0 && $('.search-result-dropdown').length) {
            this._onItemClicked(this._items[this._hilitedItemIndex]);
            this._resetSearch();
          }
          return;
        case 27: // esc
    ...
    _changeHilitedItem: function(change) {
      ...
      } else if (itemIndex >= this._items.length) {
        itemIndex = 0;
      }
      ...
      this._hilitedItemIndex = itemIndex;
      $(".search-result-row[data-index='" + itemIndex + "']").addClass("selected");
    },
    ...
    _setItems: function(items) {
      ...
      if (numItems > 0) {
        for (i = 0; i < numItems; ++i) {
            item = items[i];
            popupHtml += this._renderListItem(i, item, i == 0);
        }
      } else {
        popupHtml += this._renderNoResult();
      }
      ...
      if (numItems > 0) {
        $('.search-result-row').
          ...
          click(function() {
            self._onItemClicked(self._items[self._hilitedItemIndex]);
            self._resetSearch();
          });
    ...
    _setInteraction: function() {
      ...
      if(this.activeLayerModel) {
        ...
        for(var i = 0; i < this.groupLayer.getLayerCount(); ++i) {
          this.groupLayer.setInteraction(i, true);
        }
      }
    },
...
cdb.admin.OpenLayersMapView = cdb.geo.OpenLayersMapView.extend(GrouperLayerMapView(cdb.geo.OpenLayersMapView));
...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  setupMap: function() {
    ...
    if (!this.map_enabled) {
      ...
      this._addInfowindow(this.table, this.infowindowModel);
      this._addTooltip(this.table, this.tooltipModel);
      this._addLegends();
      this._addOverlays();
    ...
    $('.attribution a').click(function(e) {
       e.preventDefault(); 
  ...
  _addMapView: function() {
    ...
    var mapViewClass = cdb.admin.OpenLayersMapView;
    this.map.set('provider', 'openlayers'); 

    if ($.inArray("bbg_leaflet", user_data.feature_flags) !== -1) {
      mapViewClass = cdb.admin.LeafletMapView;
  ...
  _addBasemapDropdown: function() {
    ...
    if (!this.basemapDropdown) {
      ...
      if (this.vis.get("type") !== "table") {
        ...
        var $options = $('<a href="#" class="option-button dropdown basemap_dropdown" title="Click to change background map"><div class="thumb"></div>Basemap</a>');
  ...
  _exportImageClick: function(e) {
    ...
    cdb.god.trigger('closeDialogs');
    window.table.menu.trigger('hide-panel');
  ...
  _addTooltip: function(table, tooltipModel) {
    if(this.tooltip) this.tooltip.clean();
    if(table && this.mapView) {
  ...
  _addInfowindow: function(table, infowindowModel) {
    ...
    if(table && this.mapView) {
      ...
      this.infowindow.bind('openInfowindowPanel', function() {
        var infowindowLayerId = this.infowindow.model.get("layer-id");
        var layerSection = $(".layer_panel[model-id='" + infowindowLayerId + "']");
        if (layerSection.length) {
          layerSection.find(".info").click();
          this.activeLayerView.showModule('infowindow', 'fields');
          window.table.menu.trigger('show-panel');
        }
      }, this);
      ...
      table.bind('remove:row', this.updateDataLayerView, this);

      table.bind('change:dataSource', function() {
  ...
  setTooltipLayer: function(layerIndex) {
    var layer = this.map.layers.getVisibleLayerByIndex(layerIndex);
    if (!layer || (this.tooltip && this.tooltip.table.cid === layer.table.cid)) {
      return;
    }
    this._addTooltip(layer.table, layer.tooltip);
  },

  setInfowindowLayer: function(layerIndex) {
    var layer = this.map.layers.getVisibleLayerByIndex(layerIndex);
    if (!layer || (this.infowindow && this.infowindow.table.cid === layer.table.cid)) {
      this.infowindow.model.set("layer-id", layer.id);
      return;
    }
    this._hideInfowindow();
    this._addInfowindow(layer.table, layer.infowindow);
    this.infowindow.model.set("layer-id", layer.id);
  },
