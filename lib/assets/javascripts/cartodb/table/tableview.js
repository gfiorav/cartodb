...
(function() {
    ...
    cdb.admin.TableView = cdb.ui.common.Table.extend({
      ...
      _renderHeader: function() {
        ...
        thead.prepend(cdb.templates.getTemplate('table/views/table_title')({'tablename': this._titleCaseName(this.model), 'titleAlias': this._titleCaseAlias(this.model)}));
        return thead;
      },

      _titleCaseAlias: function(vis) {
        var alias = null;
        if (vis.get('synchronization') && vis.get('synchronization').from_external_source) {
          alias = (vis.get('alias')) ? Sugar.String.titleize(vis.get('alias')) : null;
        }else{
          alias = (vis.get('alias')) ? vis.get('alias') : null;
        }
        return alias;
      },
      ...
      _titleCaseName: function(vis) {
        var tableTitle = '';
        if (vis.get('synchronization') && vis.get('synchronization').from_external_source) {
          return Sugar.String.titleize(vis.get('alias'));
        }
        return vis.get('name');
      },
