 ...
 */
  ...
  var AutoCompleteInput = Backbone.View.extend({
    query: "",
    _id: -1,
    _items: [],
    _onQueryChanged: function() {},
    _renderListFooter: function() { return '' },
    _renderListItem: function() { return '' },
    _renderListHeader: function() { return '' },
    ...
    initialize: function(options) {
        this._id = AutoCompleteInput.NextId++;
        if (options.onQueryChanged !== undefined) {
          this._onQueryChanged = options.onQueryChanged;
        }
        if (options.renderListHeader !== undefined) {
          this._renderListHeader = options.renderListHeader;
        }
        if (options.renderListItem !== undefined) {
          this._renderListItem = options.renderListItem;
        }
        if (options.renderListFooter !== undefined) {
          this._renderListFooter = options.renderListFooter;
        }
        if (options.onItemClicked !== undefined) {
          this._onItemClicked = options.onItemClicked;
        }
    ...
    setItems: function(items) {
      var self = this;
      var containerId = 'ac-list-container-' + self._id;
      $('#' + containerId).remove();
      self._items = items;
      var i, numItems = items.length, item;
      var clsList = 'ac-list-' + this._id, clsItem;
      var position = this._getPosition();
      var popupHtml = '<div id="' + containerId + '" style="position:absolute;top:' + position.top + 'px;left:' + position.left + 'px;width:' + position.width + 'px;border:1px solid #ff9e24;border-radius:4px;background-color:#202020;z-index:' + position.zIndex + ';">' + 
           '<div><div style="margin:5px;text-align:right;float:right;display:inline-block;"><a id="ac-list-close-' + self._id + '" href="javascript:void(0)" class="remove" style="display:inline-block;width:13px;height:13px;background: url(&quot;' + cdb.config.get('assets_url') + '/images/sprites/common-s3966418d05.png&quot;) 0 -984px no-repeat;"></a></div>' + 
          '</div>' +
      '<div style="padding:5px;overflow-wrap:break-word;">';
      popupHtml += this._renderListHeader();
      for (i = 0; i < numItems; ++i) {
          item = items[i];
          clsItem = clsList + ' ' + clsList + '-item-' + i;
          popupHtml += this._renderListItem(i, item, clsItem);
      }
      popupHtml += this._renderListFooter();
      popupHtml += '</div></div>';
      $(popupHtml).appendTo("body");
      
      $('.' + clsList).click(function() {
          var classes = $(this).attr("class");
          var itemIndex = self._getItemIndexFromClass(classes);
          if (itemIndex != -1) {
              $('#' + containerId).remove();
              self.$el.val('');
              self._onItemClicked(self._items[itemIndex]);
          }
      });
      
      $('#ac-list-close-' + self._id).click(function() {
          $('#' + containerId).remove();
      });
    },
    ...
    _getItemIndexFromClass: function(classes) {
        var prefix = 'ac-list-' + this._id + '-item-';
        var parts = classes.split(" "), part, numParts = parts.length, i, index;
        for (i = 0; i < numParts; ++i) {
          part = parts[i];
          if (part.substring(0, prefix.length) === prefix) {
              index = parseInt(part.substring(prefix.length));
              return index;
          }
        }
        return -1;
...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  _addDataSearchACWidget: function() {
    var self = this;
    
    var acInput = new AutoCompleteInput({
      el: '#layer-search-input',
      renderListHeader: function(className) {
          return '<ul style="list-style-type:none;">';
      },
      renderListItem: function(index, item, cls) {
          var query = acInput.query;
          var queryLen = query.length;
          var name, tags = '';
          var startPos = item.name.toLowerCase().indexOf(query.toLowerCase());
          if (startPos == -1) {
              name = item.name;
          } else {
              name = item.name.substr(0, startPos) + '<b>' + item.name.substr(startPos, queryLen) + '</b>' + item.name.substr(startPos + queryLen);
          }
          
          if (item.tags != null) {
              tags = item.tags.substring(1, item.tags.length - 1);
              var tagsArray = tags.split(","), i, numTags = tagsArray.length, strTags = "", aTag;
              for (i = 0; i < numTags; ++i) {
                  aTag = tagsArray[i];
                  if (aTag[0] == '"') {
                      aTag = aTag.substring(1, aTag.length - 1);
                  }
                  strTags += aTag + ", ";
              }
              if (strTags.length > 1) {
          ...
          startPos = tags != null ? tags.toLowerCase().indexOf(query.toLowerCase()) : -1;
          if (startPos != -1) {
              tags = tags.substr(0, startPos) + '<b>' + tags.substr(startPos, queryLen) + '</b>' + tags.substr(startPos + queryLen);
          }
          var itemColor = item.on_map ? '#f55656' : (item.type === 'remote' ? '#D0BBA1' : '#FF9E24');
          var itemHtml = '<li class="' + cls + '" style="padding:5px;border-bottom:1px dotted #96754b;"><a href="javascript:void(0)" style="color:' + itemColor + '">' + name;
          if (item.description) {
              itemHtml += '<br>Desc: ' + item.description;
          }
          if (item.tags != null) {
              itemHtml += '<br>Tags: ' + tags;
          }
          itemHtml += '</a></li>';
          return itemHtml;
      },
      ...
      onQueryChanged: function(params) {
          self._doDatasetSearch(params);
      },
      onItemClicked(item) {
          if (item.type === 'remote') {
              var d = {
                  ...
                  size: undefined//mdl.get('external_source') ? mdl.get('external_source').size : undefined
              };
              cdb.god.trigger('importByUploadData', d, self);
              table.menu.show();
          } else {
              self.map.addCartodbLayerFromTable(item.name, self.user.get('username'), {
                  vis: self.vis,
                  success: function() {
    ...
    this._acInput = acInput;
    
    $("#add-layer-browse").click(function() {
      event.stopPropagation();
      table.menu._addLayerDialog();
    });
  },
  
  _doDatasetSearch: function(params) {
      var self = this;
      var query = params.query;
      var mapId = self.map.id;
      var visId = self.vis.id;
      var kNumMaxResults = 7;
      
      $.ajax({
          url: "../../api/v1/layers_list?q=" + query + "&map_id=" + mapId + "&vis_id=" + visId + "&api_key=" + self.user.attributes.api_key
      })
      .done(function(response) {
          var items = [];
          var results = response;
          var i, numResults = results.length, result;
          for (i = 0; i < numResults && i < kNumMaxResults; ++i) {
              result = results[i];
              items.push({ id: result.id, type: result.type, name: result.name, description: result.description, tags: result.tags, on_map: result.on_map });
          }
          self._acInput.setItems(items);
      })
      .fail(function() {
  ...
  enableMap: function() {
    ...
    if (this.user.featureEnabled('dataset_add_autocomplete')) {
      $("#dataset-search-input").show();
      this._addDataSearchACWidget();
    }
  },
