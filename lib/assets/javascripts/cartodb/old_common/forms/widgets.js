...
cdb.forms.Color = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this.image_property = (bbgProUI && this.options.extra) ? this.options.extra.image_property: null;
...
cdb.forms.Combo = cdb.core.View.extend({
  ...
  _getOptions: function() {
    var options = _.map(this.data, function(option) {
      if (_.isArray(option)) {
        var optionHtml = '';
        if(option.length === 3){
          optionHtml = '<option value="' + option[1] + '" alias="' + option[2] + '">' + option[0] + '</option>'
        }else{
          optionHtml = '<option value="' + option[1] + '">' + option[0] + '</option>';
        }
        return optionHtml;
      } else {
  ...
  render: function() {
    ...
    if (!this.options || !this.options.plainSelect) {
      ...
      $select.select2(combo_options).on("change", self._changeSelection);
    }
